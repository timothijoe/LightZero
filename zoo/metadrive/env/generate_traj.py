...
def get_expert_candidates(vehicle, robot_pos, path_dict):
    # naive justification, in straight highway
    degree = -robot_pos[2] * 180 / 3.1415926
    ego_speed_ms = robot_pos[3]
    rbt_pos = copy.deepcopy(robot_pos)
    ...
    # valid_idx = 0
    valid_idx_list = []
    valid_idx_list.append(1)
    appendix_list = []
    valid_left_change = False
    valid_right_change = False
    if vehicle_status[0]:
        valid_left_change = True 
        valid_idx_list.append(0)
    else:
        appendix_list.append(1)
    if vehicle_status[2]:
        valid_right_change = True 
        valid_idx_list.append(2)
    else:
        appendix_list.append(1)

    valid_ref_lanes_list = []
    append_ref_lanes_list = []
    for valid_idx in valid_idx_list:
        ref_current_lane_idx = valid_idx + current_lane_idx -1
        valid_ref_lanes_list.append(ref_current_lane_idx)
    for append_idx in appendix_list:
        append_current_lane_idx = append_idx + current_lane_idx -1
        append_ref_lanes_list.append(append_current_lane_idx)
    # ref_current_lane_idx = valid_idx + current_lane_idx -1
    ...
    for index in range(len(vehicle.navigation.current_ref_lanes)):
        # if not index == ref_current_lane_idx:
        if not index in valid_ref_lanes_list:
            continue
        long_target = long_now + forcast
        
        if index == valid_ref_lanes_list[0]:
            zengzhen = zengzhen1 
        zengzhen1 += 1
        lateral_target = 0 
        if index == 0:
    ...
    for index, key in enumerate(selected_keys):
        zt_path = path_dict[yaw_key][key]['path']
        if index == zengzhen:
            append_path = copy.deepcopy(zt_path)
        valid_idx = valid_idx_list[index]
        acc = calculate_acc_for_maintaing_distance(vehicle_near_distance[valid_idx], ego_speed_ms, vehicle_near_speed[valid_idx]/3.6, desired_distance = 12, t=2)
        zt_traj =  select_trajectory_from_path(zt_path, ego_speed_ms, acc)
        zt_traj_list.append(zt_traj)
    if not valid_left_change:
        acc = -1.5
        zt_traj =  select_trajectory_from_path(zt_path, ego_speed_ms, acc)
        zt_traj_list.append(zt_traj)
    if not valid_right_change:
        acc = -2.5
        zt_traj =  select_trajectory_from_path(zt_path, ego_speed_ms, acc)
        zt_traj_list.append(zt_traj)
    return zt_traj_list 